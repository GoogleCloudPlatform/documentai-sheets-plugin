...
/**
 * The extension for providing additional actions for ResultFramework on AppScript.
...
class AppScriptExtension extends Extension {
   ...
   *     property in coreConfig loaded from src/core.js.
   */
  constructor(config, envVars) {
    ...
    this.coreVersion = config.coreVersion || 'latest';
  ...
  trackAction(trackingType, sheetId, result) {
    ...
    let referral = this.coreVersion || 'latest';
    let url;

    // FIXME: Load the list of data sources from coreConfig instead of hardcoded.
